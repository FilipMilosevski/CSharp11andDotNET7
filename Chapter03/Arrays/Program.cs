...
//allocationg memory for four strings in an array
names = new string[4];    // first way
...
string[] names2 = new[] { "Kate", "Jack", "Rebecca", "Tom" }; //second way in one line
//Looping through the names
for (int i = 0; i < names2.Length; i++)
{
    WriteLine(names2[i]); // output the item at index position i
}
...
WriteLine($"Lower bound of the first dimension is: {grid1.GetLowerBound(0)}");  //od golemite zagradi
WriteLine($"Upper of the first dimension is: {grid1.GetUpperBound(0)}"); //do golemite zagradi
WriteLine($"Lower bound of the SECOND dimension is: {grid1.GetLowerBound(1)}"); //od malite
WriteLine($"Upper bound of the SECOND dimension is: {grid1.GetUpperBound(1)}"); //do malite
...
for (int array = 0; array <= jagged.GetUpperBound(0); array++)
{
    ...
    //WriteLine($"Upper bound of array {array} is: {jagged[array].GetUpperBound(0)}"); bez prviot WL
}
...
//STEP INTO PA STEP OVER INTO OVER SO DEBUG